...
/**
 * This class is describes an individual audio track that will be handled by the
 * audio handler
 * 
 * @author Alex Cash
...
public class Audio {
	...
	/* Variables */
	/**
	 * Double variable used to store the volume level so as to restore it after
	 * un-muting.
	 */
	...
	/**
	 * Boolean variable used to indicate whether the audio was paused using the
	 * "Pause" button.
	 */
	...
	/**
	 * Integer variable used to define the width of buttons used for the audio
	 * player.
	 */
	 ...
	 * 
	 * @param nGroup
	 *            The group this audio track goes on.
	 * 
	 * @param x
	 *            The x coordinate of the top left of the audio player relative
	 *            to the group's origin.
	 * 
	 * @param y
	 *            The y coordinate of the top left of the audio player relative
	 *            to the group's origin.
	 * 
	 * @param width
	 *            The width of the audio player in pixels.
	 * 
	 * @param sourceFile
	 *            Absolute path of the audio track as a string. Can be a local
	 *            file path or a web address beginning with 'http'
	 * 
	 * @param autoPlay
	 *            If true the video plays immediately
	 * 
	 * @param loop
	 *            If true the audio track loops to the beginning when it ends
	 * 
	 * @param visibleControls
	 *            If true, there are visible buttons displayed. If this is not
	 *            true, autoPlay is set to true
	 * 
	 * @param playButtonOnly
	 *            If true, the only visible button is a play/pause button. If
	 *            false and visibleControls is true, all control buttons are
	 *            visible
	 * 
	 */
	public Audio(Group nGroup, float x, float y, float width, String sourceFile, boolean autoPlay, boolean loop,
			boolean visibleControls, boolean playButtonOnly) {
		/* Set the group reference */
		this.group = nGroup;

		/*
		 * If the width is less than our defined minimum, set width to our
		 * minimum
		 */
		if (width < 130) {
			width = 130;
		...
		/*
		 * Create and arrange the VBoxes and HBoxes required to lay out the
		 * audio player
		 */
		...
		playSeekHbox.setMinHeight(45); // To account for the size of the buttons
										// and the slider, and their respective
										// positions in their boxes
		...
		if (sourceFile.startsWith("http")) {
			...
			if (!file.exists()) {
				/*
				 * Add a label to notify the user that the file could not be
				 * found
				 */
				...
				mediaExists = false;
				return;
			} else if (!file.getAbsolutePath().endsWith(".mp3") && !file.getAbsolutePath().endsWith(".wav")) {
		...
		/*
		 * If the loop variable is true, set the player to cycle through the
		 * audio indefinitely
		 */
		if (loop) {
		...
		/*
		 * Once the player is ready, get the information about the duration of
		 * the audio file
		 */
		...
		player.setOnReady(new Runnable() {
			...
			public void run() {
				...
				/*
				 * Set the sizes of the label VBoxes once the other boxes are
				 * initialised
				 */
		...
		/*
		 * Add a listener to the time property of the player for scanning and
		 * scrolling
		 */
		...
		if (visibleControls == true && playButtonOnly == false) { // Add all
																	// visible
																	// controls
		...
		} else if (visibleControls == true && playButtonOnly == true) { // Add
																		// just
																		// the
																		// play/pause
																		// button
	...
	private static boolean mediaExists(String url) {
		try {
			...
			/*
			 * If a response code of 200 (okay) is received, the media is
			 * available
			 */
	...
	/**
	 * ChangeListener that listens to the current time value property of the
	 * mediaplayer. It updates the slider along the track as the audio plays and
	 * calls for the labels to be updated.
	 */
	public class progressUpdater implements ChangeListener<Duration> {
		...
		public void changed(ObservableValue<? extends Duration> arg0, Duration arg1, Duration arg2) {
			...
			/*
			 * If the user is not manually scanning, update the slider
	...
	/**
	 * Method to update the labels dependent on current progress through the
	 * media and the total length of the media.
	 */
	public void updateLabels() {
		...
		/*
		 * If the audio is longer than 99 minutes, increase to 3 digits to
		 * accomodate
		 */
	...
	/**
	 * EventHandler that listens to mouse inputs on the slider. Listens for both
	 * mouse presses and releases on the slider thumb.
	 */
	...
	/**
	 * EventHandler for presses of the controls button. This will remove visible
	 * controls when pressed, or re-add invisible controls when pressed.
	 */
	...
	/**
	 * Method to actually change the mediaplayers position through the media
	 * when the user seeks through the media.
	 * 
	 * @param percentSeek
	 *            the value to which the user requests the audio seek to.
	 */
