...
/**
 * This class encapsulates an individual video within the video handler.
 * 
...
public class Video {
	 ...
	 * 
	 * @param nGroup
	 *            The group this video goes on.
	 * 
	 * @param x
	 *            The x coordinate of the top left of the video relative to the
	 *            group's origin.
	 * 
	 * @param y
	 *            The y coordinate of the top left of the video relative to the
	 *            group's origin.
	 * 
	 * @param width
	 ...
	 * 
	 * @param sourcefile
	 *            Absolute path of the video as a string. Can be a local file
	 *            path or a web address beginning with 'http'
	 * 
	 * @param autoPlay
	...
	public Video(Group nGroup, Stage stage, float x, float y, float width, String sourcefile, boolean autoPlay,
		...
		stopImage = new ImageView(
			new Image(getClass().getResourceAsStream("Stop_ST_CONTENT_RECT_Transparent_L-01.png")));
		fullscreenImage = new ImageView(new Image(getClass().getResourceAsStream(
			"FullscreenON_ST_CONTENT_RECT_Transparent_L-01.png")));
		fullscreenOffImage = new ImageView(new Image(getClass().getResourceAsStream(
			"FullscreenOFF_ST_CONTENT_RECT_Transparent_L-01.png")));
		...
		if (sourcefile.startsWith("http")) {
			...
			if (!file.exists()) {
				...
				group.getChildren().add(label);
				return;
			} else if (!file.getAbsolutePath().endsWith(".mp4") && !file.getAbsolutePath().endsWith(".flv")) {
				/* Return to halt the creation of this video */
				return;
			}
	 ...
	 * 
	 * @param percent
	 *            - The new position in the video as a percentage of the video
	 *            length.
	 */
	public void scan(double percent) {
		...
		if (mediaPlayer.getStatus() == Status.STOPPED || mediaPlayer.getStatus() == Status.READY) {
			mediaPlayer.pause();
		}
	...
	/**
	 * Handles adding and removing the controls from the video when the mouse
	 * enters and leaves the frame.
	 */
	...
	/**
	 * Handles performing actions on the video based on the buttons pressed by
	 * the user.
	 */
	...
	/**
	 * Handles moving through the video based on the user moving the scan bar.
	 */
	...
	/**
	 * Listener for changes to the current position in the video to update the
	 * scan bar as the video plays.
	 */
	...
	public class VolumeListener implements ChangeListener<Number> {
		@Override
		public void changed(ObservableValue<? extends Number> val, Number oldVal, Number newVal) {
	...
	/**
	 * Stops the media player when the it reaches the end of the current video.
	 * This resets it back to the beginning of the clip.
	 */
	...
	/**
	 * Listener for changes in the media player status. Used to update the
	 * buttons and time stamps.
	 */
