...
public class fakeClients extends Application {
	...
	private class buttonHandler implements EventHandler<ActionEvent> {
		...
		public void handle(ActionEvent e) {
			...
			if (btnPressed.getId().equals("connect")) {
				...
				for(int i = 0; i < 250; i++) {
					clients.add(Integer.toString(i + 2) + "." + "5" + "." + Integer.toString(i + 2) + "." + Integer.toString(i + 2));
				}
				for(int i = 0; i < 250; i++) {
					clients.add(Integer.toString(i + 2) + "." + "6" + "." + Integer.toString(i + 2) + "." + Integer.toString(i + 2));
				}
				
				boolean answered = true;
				
				for(int i = 0; i < 20; i++) {
					if((((int)Math.round(Math.random() * 10)) == 7)&&answered) {
						client.sendToServerWithoutIP("25.7.25.27" + ":" + "0");
						answered = false;
					}
					client.sendToServerWithoutIP(clients.get(0) + ":" + "2");
					clients.remove(0);
					try {
				...
				for(int i = 0; i < 480; i++) {
					String ans = Integer.toString((int)Math.round(Math.random()*3));
					while(ans.equals("2")) {
						ans = Integer.toString((int)Math.round(Math.random()*3));
					}
					answers.put(clients.get(i), ans);
				}
				try {
				...
				while(clients.size() > 0) {
					int clientIndex = (int)Math.floor(Math.random() * clients.size());
					String clientString = clients.get(clientIndex);
					client.sendToServerWithoutIP(clientString + ":" + answers.get(clients.get(clientIndex)));
					
					String oldAns = answers.get(clients.get(clientIndex));
					while((answers.get(clients.get(clientIndex)).equals(oldAns))) {
						answers.put(clients.get(clientIndex), Integer.toString((int)Math.round(Math.random()*3)));
					}
